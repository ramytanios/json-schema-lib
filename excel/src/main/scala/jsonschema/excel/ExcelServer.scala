...
object ExcelServer:
  ...
  def server(excel: Excel, port: Port = defaultPort): Resource[IO, Server] =
    for
      ...
      srv <- EmberServerBuilder
        ...
        .withPort(port)
        .withHttpApp(routes(js, json, html).orNotFound)
        .build
    yield srv

  private def routes(js: String, json: String, html: String): HttpRoutes[IO] =
    HttpRoutes.of[IO]:
